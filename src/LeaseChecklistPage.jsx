...
const DUMMY_ITEM = {
  aptNm: "명지힐하우스",
  address: "서울시 서대문구 남가좌동 29-1",
  deposit: 30000000,
  monthlyRent: 840000,
  maintenanceFee: 50000,
  사진: "",
  채광: 3, 수압: 5, 청결: 5, 옵션: 5, 보안: 3,
  소음: 3, 주변환경: 5, 분리수거: 3,
  isElevatorAvailable: true, isBalconyAvailable: false, isPetAllowed: false,
  notes: ""
};

export default function LeaseChecklistPage() {
  const location = useLocation();
  const item = location.state?.item || DUMMY_ITEM;

  // 초기 계약형: 데이터에 월세가 있으면 'monthly', 없으면 'jeonse'
  const initialContract = item?.monthlyRent > 0 ? "monthly" : "jeonse";

  // === 상태
  const [contract, setContract] = useState(initialContract); // 'monthly' | 'jeonse'
  const [enabled, setEnabled] = useState(true);
  const [openIds, setOpenIds] = useState(() => new Set());

  // 공통/월세/전세 별도 체크 저장
  const initMap = (arr) => Object.fromEntries(arr.map(it => [it.id, false]));
  const [checked, setChecked] = useState({
    common: initMap(COMMON),
    monthly: initMap(MONTHLY),
    jeonse: initMap(JEONSE),
  });

  const [whyModal, setWhyModal] = useState(null);
  const KEY = "leaseChecklist_toggle_v1";
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (parsed.contract === "monthly" || parsed.contract === "jeonse") setContract(parsed.contract);
        if (parsed.checked && typeof parsed.checked === "object") {
          setChecked((prev) => ({
            common: { ...prev.common, ...parsed.checked.common },
            monthly: { ...prev.monthly, ...parsed.checked.monthly },
            jeonse: { ...prev.jeonse, ...parsed.checked.jeonse },
          }));
        }
      }
    } catch (e) {
  ...
  const onSave = () => {
    localStorage.setItem(KEY, JSON.stringify({ enabled, contract, checked }));
    alert("저장되었습니다.");
  };
  ...
  // 체크 토글 (id prefix로 저장 위치 구분)
  const toggleCheck = (id) => {
    if (!enabled) return;
    setChecked(prev => {
      const bucket =
        id.startsWith("c") ? "common" :
        id.startsWith("m") ? "monthly" : "jeonse";
      return {
        ...prev,
        [bucket]: { ...prev[bucket], [id]: !prev[bucket][id] }
      };
    });
  };
  ...
  return (
    <Wrap>
      ...
      {/* 상단 탭 (활성시 글씨만 파랑, 밑줄 없음) */}
      <Tabs>
        <TabLink to="/homedetailpage" end className={({isActive})=>isActive?"active":""}>상세보기</TabLink>
        <TabLink to="/leasechecklistpage" end className={({isActive})=>isActive?"active":""}>집 계약 체크리스트</TabLink>
      </Tabs>
      ...
      {/* 전세/월세 토글 */}
      <SegmentWrap role="tablist" aria-label="contract type">
        <SegBtn
          role="tab"
          aria-selected={contract === "monthly"}
          $active={contract === "monthly"}
          onClick={()=>setContract("monthly")}
        >
          월세
        </SegBtn>
        <SegBtn
          role="tab"
          aria-selected={contract === "jeonse"}
          $active={contract === "jeonse"}
          onClick={()=>setContract("jeonse")}
        >
          전세
        </SegBtn>
      </SegmentWrap>
        ...
        <List>
          ...
          {specificList.map(it=>(
            <ChecklistItem key={it.id} item={it}
              ...
              checked={
                contract==="monthly"
                  ? !!checked.monthly[it.id]
                  : !!checked.jeonse[it.id]
              }
              onToggleOpen={()=>toggleOpen(it.id)}
              onToggleCheck={()=>toggleCheck(it.id)}
              onShowWhy={()=>showWhy(it)} />
...
const TabLink = styled(NavLink)`
  display:block;text-align:center;padding:10px 0;font-weight:700;
  color:${C.sub};text-decoration:none;cursor:pointer;
  &.active{ color:${C.blue}; } /* 밑줄 없이 글씨만 파랑 */
`;

const SegmentWrap = styled.div`
  display:grid;grid-template-columns:1fr 1fr;
  gap:8px;padding:0 ${S.padX}px;margin:12px 0;
`;
const SegBtn = styled.button`
  height:38px;border-radius:10px;border:1px solid ${p=>p.$active?C.blue:C.line};
